...
const URL =
  'http://192.168.0.106:8008/setup/assistant/alarms';
const TIME_OF_UPDATE = 1000;
const PATH_TO_ICON = path.join(
  __dirname,
  '../../iconTemplate.png'
);
...
const transformGoogleDataToTrayTitle = flow(
  ...
  timers => timers[0],
  timer =>
    !!timer ? getTimerHumanDuration(timer) : undefined
);

const setTrayTitle = ({ tray, googleData }) => {
  const title =
    transformGoogleDataToTrayTitle(googleData) || '—';
  tray.setTitle(title);
};
